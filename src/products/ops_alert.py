...
Evaluates reconciliation health for a reference_date and writes:
  - {output_dir}/{date}_chart.svg   — bar chart of category rates
...
def run(
    ...
    Args:
        ...
        output_dir:     Directory where chart and payload files are written.
    ...
    try:
        ...
        chart_path = _write_chart(ref_date, daily, out_dir)
        payload = _block_kit(ref_date, run_status, daily, alerts)
        payload_path = out_dir / f"{ref_date}_alert.json"
        ...
        logger.info(
            "Ops alert written — level=%s reference_date=%s artifacts=[%s, %s]",
            level,
            ref_date,
            chart_path.name,
            payload_path.name,
...
def _write_chart(ref_date: str, daily: list[dict], out_dir: Path) -> Path:
    svg = _svg_bar_chart(
        title=f"Reconciliation Rates — {ref_date}",
        bars=[(r["category"], r["pct_of_total"]) for r in daily],
    )
    path = out_dir / f"{ref_date}_chart.svg"
